...
Astro 6 기반의 정적 개인 블로그입니다. Obsidian에서 작성한 Markdown을 `content/post`와 `content/wiki`에 저장하고, Astro 페이지에서 읽어 블로그 글과 Wiki로 보여줍니다.
...
- `src/pages/wiki/index.astro`: Wiki 목록 페이지입니다. `content/wiki`의 Markdown을 카드형 목록으로 보여주고 hover/focus 설명을 띄웁니다.
- `src/pages/wiki/[slug].astro`: Wiki 상세 페이지입니다. 게시글 상세 페이지와 같은 Markdown 렌더링, 코드 블럭 복사, 이미지, Mermaid 렌더링 흐름을 사용합니다.
...
- `content/templates`: Obsidian용 글/Wiki 템플릿입니다.
...
Wiki 항목은 `content/wiki/**/*.md`에 둡니다. 현재 권장 frontmatter:
...
- `tags`는 Wiki 항목과 같은 대상을 가리키는 다른 표기로 사용됩니다. 하이라이팅은 대소문자를 구분하지 않으므로 영문 표기는 소문자로 관리합니다. 예를 들어 `markdown`, `md`, `마크다운`처럼 완전히 같은 개념을 가리키는 표기만 넣습니다.
...
- `/wiki/` Wiki 페이지는 홈 보기 전환 버튼의 영향을 받지 않습니다.
- 사이드바에서 태그 필터를 선택했을 때 같은 태그, title, slug를 가진 Wiki 항목이 있으면 해당 항목의 `define`과 상세 페이지 바로가기 버튼을 보여줍니다.
