...
- `src/pages/index/index.astro`: 개념 사전 목록 페이지입니다. `content/index`의 Markdown을 카드형 사전으로 보여주고 hover/focus 설명을 띄웁니다.
- `src/pages/index/[slug].astro`: 개념 사전 상세 페이지입니다. 게시글 상세 페이지와 같은 Markdown 렌더링, 코드 블럭 복사, 이미지, Mermaid 렌더링 흐름을 사용합니다.
...
- `define`이 index 카드와 상세 페이지의 핵심 설명으로 사용됩니다.
- `define`이 없으면 `description`, 본문 첫 줄 순서로 설명을 사용합니다.
- 파일명에서 확장자를 제거한 값이 기본 항목명입니다.
- `tags`에 다른 index의 slug 또는 title을 넣으면 상세 페이지 하단에 관련 index 링크가 표시됩니다.
- `/index/`의 카드는 `/index/[slug]/` 상세 페이지로 이동합니다.
...
- 사이드바에서 태그 필터를 선택했을 때 같은 태그, title, slug를 가진 index가 있으면 해당 index의 `define`과 상세 페이지 바로가기 버튼을 보여줍니다.
