...
`Loading / Success / Error`로 부족한 경우도 있다. 결과가 비어 있는 화면, 목록을 유지한 채 새로고침하는 화면이 대표적이다.

여기서 다시 플래그를 덧붙이면 처음 문제로 돌아간다. sealed의 성질을 유지한 채 조정한다.

별개의 화면이면: 새로운 종류로 승격시킨다.

```kt
data object Empty : ProductUiState
```

특정 상태 안에서만 의미 있으면: 그 하위 타입 안에 필드로 둔다.
...
`isRefreshing`은 `Success`일 때만 존재하므로 "로딩 중인데 새로고침 중" 같은 모순이 안 생긴다. 플래그가 문제였던 게 아니라 **아무 상태에서나 켜지는 플래그**가 문제였고, 하위 타입 안에 가두면 그 값은 다시 실제 상태와 맞아떨어진다.
...
Repository나 UseCase에서 잡은 예외를 이 도메인 에러로 **번역**해서 올려보낸다. 그러면 화면은 프레임워크의 예외 타입이 아니라 앱이 정의한 닫힌 종류를 기준으로 대응할 수 있다.

`when` 망라 검사도 그대로 작동한다.
